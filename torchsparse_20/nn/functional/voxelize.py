...
class VoxelizeFunction(Function):
    ...
    def forward(ctx, feats: torch.Tensor, coords: torch.Tensor,
        ...
        if feats.device.type == 'cuda':
            output = torchsparse_20.backend.voxelize_forward_cuda(
                feats, coords, counts)
        elif feats.device.type == 'cpu':
            output = torchsparse_20.backend.voxelize_forward_cpu(
                feats, coords, counts)
        else:
            device = feats.device
            output = torchsparse_20.backend.voxelize_forward_cpu(
    ...
    def backward(ctx, grad_output: torch.Tensor):
        ...
        if grad_output.device.type == 'cuda':
            grad_feats = torchsparse_20.backend.voxelize_backward_cuda(
                grad_output, coords, counts, input_size)
        elif grad_output.device.type == 'cpu':
            grad_feats = torchsparse_20.backend.voxelize_backward_cpu(
                grad_output, coords, counts, input_size)
        else:
