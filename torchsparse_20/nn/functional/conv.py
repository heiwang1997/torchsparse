...
class ConvolutionFunction(Function):
    ...
    def forward(
        ...
        if input.device.type == 'cuda':
            output = torchsparse_20.backend.convolution_forward_cuda(
                input,
                weight,
                nbmaps,
                nbsizes.cpu(),
            ...
            torchsparse_20.backend.convolution_forward_cpu(input, output, weight,
                                                        nbmaps, nbsizes.cpu(),
                                                        transposed)
        else:
    ...
    def backward(ctx, grad_output: torch.Tensor):
        ...
        if grad_output.device.type == 'cuda':
            torchsparse_20.backend.convolution_backward_cuda(
                input,
                grad_input,
            ...
            torchsparse_20.backend.convolution_backward_cpu(
                input,
                grad_input,
                grad_output.contiguous(),
                weight,
