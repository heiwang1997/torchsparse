...
def sphashquery(queries: torch.Tensor,
    ...
    if queries.device.type == 'cuda':
        output = torchsparse_20.backend.hash_query_cuda(queries, references,
                                                     indices)
    elif queries.device.type == 'cpu':
        output = torchsparse_20.backend.hash_query_cpu(queries, references,
                                                    indices)
    else:
        device = queries.device
        output = torchsparse_20.backend.hash_query_cpu(queries.cpu(),
                                                    references.cpu(),
