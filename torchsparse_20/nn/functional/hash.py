...
def sphash(coords: torch.Tensor,
    ...
    if offsets is None:
        if coords.device.type == 'cuda':
            return torchsparse_20.backend.hash_cuda(coords)
        elif coords.device.type == 'cpu':
            return torchsparse_20.backend.hash_cpu(coords)
        else:
            device = coords.device
            return torchsparse_20.backend.hash_cpu(coords.cpu()).to(device)
    else:
        ...
        if coords.device.type == 'cuda':
            return torchsparse_20.backend.kernel_hash_cuda(coords, offsets)
        elif coords.device.type == 'cpu':
            return torchsparse_20.backend.kernel_hash_cpu(coords, offsets)
        else:
            device = coords.device
            return torchsparse_20.backend.kernel_hash_cpu(coords.cpu(),
